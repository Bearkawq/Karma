...
def build_model_status_text(manager, slot_mgr) -> str:
    ...
    for row in rows:
        ...
        lines.append(
            ...
            f"present_on_disk: {row['installed_in_ollama']} | installed: {row['installed_in_ollama']} | "
            f"loaded(warm): {row['warm_now']} | warm_now: {row['warm_now']} | "
    ...
    for model_id in SMALL_MODEL_POOL:
        ...
        lines.append(
            f"- {model_id} | present_on_disk: {matched is not None} | installed: {matched is not None} | "
            f"ollama: {matched or '<missing>'} | loaded(warm): {loaded} | warm_now: {loaded}"
        )
...
def format_readiness_report(report: Dict[str, Any]) -> str:
    ...
    for model in report["small_models"]:
        lines.append(
            f"- {model['model']} | present_on_disk: {model['present']} | installed: {model['present']} | "
            f"ollama: {model['ollama_model'] or '<missing>'} | loaded(warm): {model['warm_now']} | warm_now: {model['warm_now']}"
        )
    lines.append("")
    ...
    for row in report["roles"]:
        ...
        lines.append(
            ...
            f"ollama: {row['ollama_model'] or '<missing>'} | present_on_disk: {row['installed_in_ollama']} | installed: {row['installed_in_ollama']} | "
            f"loaded(warm): {row['warm_now']} | warm_now: {row['warm_now']} | "
